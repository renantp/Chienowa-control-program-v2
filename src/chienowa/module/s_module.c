...
int s31_salt_tank_data_check(void){
	union {
		...
		uint8_t raw;
			...
			.fl7 = FL7 == LEVEL_ON ? 1U : 0U,
			.fl8 = FL8 == LEVEL_ON ? 1U : 0U,
			.rsvd = 0b000000 };
	switch (data.raw) {
		case 0:
			return 0;
...
int s21_acid_tank_data_check(void) {
	union {
		...
		uint8_t raw;
			...
			.fl1 = FL1 == LEVEL_ON ? 1U : 0U,
			.fl2 = FL2 == LEVEL_ON ? 1U : 0U,
			.fl3 = FL3 == LEVEL_ON ? 1U : 0U,
			.rsvd = 0b00000 };
	switch (data.raw) {
	case 0:
		return 0;
...
int s11_alkali_tank_data_check(void) {
	union {
		...
		uint8_t raw;
			...
			.fl4 = FL4 == LEVEL_ON ? 1U : 0U,
			.fl5 = FL5 == LEVEL_ON ? 1U : 0U,
			.fl6 = FL6 == LEVEL_ON ? 1U : 0U,
			.rsvd =	0b00000 };
	switch (data.raw) {
	case 0:
		return 0;
